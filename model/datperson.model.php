...
class ModelPersonalData {
    ...
    public static function getDataTable() {
        $sql = "SELECT persona.CvPerson, persona.Curp, nombre.DsNombre 'Nombre', apepat.DsApellido 'ApePat', apemat.DsApellido 'ApeMat', persona.Edad, estado.DsEstado 'Estado', municipio.DsMunicipio 'Municipio', colonia.DsColonia 'Colonia', calle.DsCalle 'Calle', persona.Numero, persona.Cp, tipPerson.DsTipPerson 
                FROM cNombre nombre, cApellido apepat, cApellido apemat, mPersona persona, cEstado estado, cMunicipio municipio, cColonia colonia, cCalle calle, cTipPerson tipPerson
                WHERE 
                ...
                persona.CvCalle = calle.CvCalle AND
                persona.CvTipPerson = tipPerson.CvTipPerson";
    ...
    /** Add a New Person */
    public static function addNewPerson($TipPerson, $Name, $LastName, $MotherLastName, $Date, $Age, $Gender, $Phone, $Email, $Curp, $Rfc, $State, $Municipality, $Suburb, $Street, $Number, $PostalCode) {
    ...
    public static function deletePerson($CvPerson) {
        $sql = 'DELETE FROM mPersona WHERE CvPerson = :CvPerson;
                SET @num := 0;
                UPDATE mPersona SET CvPerson = @num := (@num+1);
                ALTER TABLE mPersona AUTO_INCREMENT = 1';
    ...
    public static function editPerson($CvPerson, $TipPerson, $Name, $LastName, $MotherLastName, $Date, $Age, $Gender, $Phone, $Email, $Curp, $Rfc, $State, $Municipality, $Suburb, $Street, $Number, $PostalCode) {
        $sql = 'UPDATE mPersona SET CvTipPerson = :CvTipPerson, Curp = :Curp, Rfc = :Rfc, Email = :Email, CvNombre = :CvNombre, CvApePat = :CvApePat, CvApeMat = :CvApeMat, FecNac = :FecNac, Edad = :Edad, CvGenero = :CvGenero, Telefono = :Telefono, CvEstado = :CvEstado, CvMunicipio = :CvMunicipio, CvColonia = :CvColonia, CvCalle = :CvCalle, Numero = :Numero, Cp = :Cp
    ...
    public static function existsCURP($CURP, $TipPerson) {
        $sql = "SELECT persona.CvPerson, nombre.DsNombre, apepat.DsApellido from mpersona persona, cNombre nombre, cApellido apepat WHERE nombre.CvNombre = persona.CvNombre AND apepat.CvApellido = persona.CvApePat AND Curp = :Curp AND CvTipPerson = :CvTipPerson";
