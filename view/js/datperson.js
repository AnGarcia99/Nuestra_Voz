...
function repeatCURP() {
    if ($('#nuevo').text() == 'Grabar') {
        ...
        if (($('#curp').val()).length == 18 && ($('#TipPerson').val())>0) {

            let curp = $('#curp').val();
            let tipperson = $('#TipPerson').val();
...
function addNewPerson() {
    ...
    if ($('#nuevo').text() == 'Nuevo') {
        ...
        $('.filasTabla').removeClass('seleccionada');
...
function deletePerson() {
    ...
    swal({
        type: "warning",
        title: '¿Estas seguro?',
        text: 'Se eliminara a ' + $($('#' + id).children()[1]).text() + ' ' + $($('#' + id).children()[2]).text(),
        background: '#222D32',
        showCancelButton: true,
        confirmButtonColor: '#3085d6',
        cancelButtonColor: '#d33',
            ...
            $.ajax({
                ...
                if (JSON.parse(res)) {
                    swal({
                        type: "error",
                        title: "ERROR",
                        text: "¡La persona esta siendo utilizada en mUsuarios!",
                        background: '#222D32',
...
function clearErrors() {
    const errors = ['CurpError', 'RfcError', 'TipPersonError', 'NombreError', 'ApePatError', 'ApeMatError', 'DateError', 'GeneroError', 'MatriculaError', 'EmailError', 'CalleError', 'ColoniaError', 'MunicipioError', 'EstadoError', 'CpError'];
...
function validarCaracteresCurpRfc() {
    $('#curp').bind('keypress', function(event) {
    ...
    $('#rfc').bind('keypress', function(event) {
        var regex = new RegExp("^[A-ZÑ\x26]{3,4}([0-9]{2})(0[1-9]|1[0-2])(0[1-9]|1[0-9]|2[0-9]|3[0-1]))((-)?([A-Z\d]{3})+$"); 
        var key = String.fromCharCode(!event.charCode ? event.which : event.charCode);
        if (!regex.test(key)) {
            event.preventDefault();
            return false;
...
/** ------------- Validar campos de fechas (FecNac, Edad) --------------  */
...
/** ------------- END Validar campos de fechas (FecNac, Edad) --------------  */
